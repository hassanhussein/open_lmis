...
public interface InspectionMapper {
    ...
    @Select("select r.poNumber,i.varNumber,i.id,asnNumber,A.modifiedDate asnDate, i.receiveNumber receiptNumber,r.modifiedDate receiptDate, i.status,i.status as customStatus from inspections i\n" +
            "JOIN receives r on i.receiveid = r.id \n" +
            "JOIN receive_LINE_ITEMS LI on r.id = li.receiveId\n" +
            "JOIN Asns A ON a.id = r.asnId ")
    ...
    class SelectInspection {
        ...
        public static String getSearchBy(Map<String, Object> params) {
            ...
            sql.append("WITH Q AS (select i.id,asnNumber,i.varNumber,r.poNumber,A.modifiedDate asnDate, i.receiptNumber,r.modifiedDate receiptDate, i.status,i.status as customStatus from inspections i\n" +
                    "JOIN receives r on i.receiveid = r.id \n" +
                    "JOIN receive_LINE_ITEMS LI on r.id = li.receiveid\n" +
                    "JOIN Asns A ON a.id = r.asnId )\n" +
                    "SELECT * FROM Q WHERE ");
