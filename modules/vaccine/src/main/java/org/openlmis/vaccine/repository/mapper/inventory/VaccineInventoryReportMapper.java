...
public interface VaccineInventoryReportMapper {
    ...
    @Select("select max(f.id) facilityId,max(p.id) productid,max(d.periodid) period,  d.status,f.name toFacility, p.primaryname product, sum(dl.quantity) quantity from vaccine_distribution_line_items dl\n" +
            ...
            "group by f.name, p.primaryname,d.status\n" +
