...
DECLARE
  ...
  cur_receive CURSOR(p_insid INTEGER)
    	FOR select r.id, r.id receiveid, rli.productid, rli.quantitycounted, rli.boxcounted,  t.id lotid, rl.lotnumber, rli.lotflag
					from receives r
    ...
    -- exit when no more row to fetch
      ...
      v_lotflag := rec_receive.lotflag;
			...
			INSERT INTO inspection_lots(
			 inspectionlineitemid, lotnumber, countedquantity, createddate, modifieddate)
			VALUES (v_inspectonlineitemid, v_lot_number, v_lot_quantity, NOW(), NOW());
