...
select
 ...
 where (r.id, r.facilityid)  in (select MAX(id), facilityid from requisitions where status='RELEASED' group by facilityid)
 group by rli.productcode, p.tracer, p.primaryname, gz.district_name,  gz.region_name
WITH NO DATA;
