-- View: public.vw_timeliness_report

 DROP VIEW if exists public.vw_timeliness_report;

CREATE OR REPLACE VIEW public.vw_timeliness_report AS
 SELECT requisitions.programid,
  ...
  WHERE (requisitions.status::text = ANY (ARRAY['RELEASED_NO_ORDER'::character varying::text, 'RELEASED'::character varying::text, 'AUTHORIZED'::character varying::text, 'IN_APPROVAL'::character varying::text, 'APPROVED'::character varying::text, 'RELEASED'::character varying::text])) AND facilities.active = true AND requisitions.emergency = false
...
ALTER TABLE public.vw_timeliness_report
    OWNER TO postgres;
