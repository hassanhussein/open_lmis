DELETE FROM configuration_settings WHERE key = 'REDESIGNED_DISTRICT_REPORTING_CUT_OFF_DATE';
INSERT INTO configuration_settings(key, value, name, description, groupname, valuetype)
values('REDESIGNED_DISTRICT_REPORTING_CUT_OFF_DATE',10,'Cut-off date to track distict timeliness reporting','Cut-off date to track district timeliness reporting.','GENERAL','TEXT');

DELETE FROM configuration_settings WHERE key = 'REDESIGNED_MSD_ZONE_REPORTING_CUT_OFF_DATE';
INSERT INTO configuration_settings(key, value, name, description, groupname, valuetype)
values('REDESIGNED_MSD_ZONE_REPORTING_CUT_OFF_DATE',21,'Cut-off date to track MSD Zone Timeliness Reporting for redesigned system','Cut-off date to track MSD Zone Timeliness Reporting for redesigned.','GENERAL','TEXT');
...
CREATE MATERIALIZED VIEW public.mv_dashboard_timeliness_report AS
 ...
 SELECT requisitions.programid,
  ...
  WHERE (requisitions.status::text = ANY (ARRAY['AUTHORIZED','IN_APPROVAL'::character varying::text, 'APPROVED'::character varying::text, 'RELEASED'::character varying::text])) AND facilities.active = true AND requisitions.emergency = false
  GROUP BY requisitions.status, requisitions.createddate, pp.enddate, requisitions.id, requisitions.programid, requisitions.periodid, rgps.scheduleid, facilities.geographiczoneid, facilities.name,
  facilities.code, facilities.id, facility_types.id, facility_types.name
