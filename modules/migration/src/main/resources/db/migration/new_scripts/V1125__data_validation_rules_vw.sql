...
CREATE
OR
replace FUNCTION PUBLIC.fn_rule_received_stock_indivisible_by_msd_unit(v_rnr_id integer)
 returns table ( maxStockRule boolean, message text)
AS $$
DECLARE
   result record;
BEGIN
  RETURN QUERY select CASE WHEN (rli.quantityreceived%p.packsize) > 0 THEN 'Quantity received indivisible by MSD unit of measure (UOM)'::text ELSE 'SUCCESS'::text END
  from requisition_line_items rli
join products p on p.code= rli.productcode
where rli.rnrid=v_rnr_id;
END;
$$
LANGUAGE plpgsql;
...
BEGIN
         RETURN QUERY
select  'stockout_days'as rule, * from fn_stockout_days_rule( v_rnr_item_id) UNION ALL
select  'maxmum_stock' as rule, * from fn_maximum_stock_rule(v_facility_id,v_rnr_id,v_program_id, v_product) UNION ALL
select  'minimum_stock' as rule, * from fn_minimum_stock_rule(v_facility_id,v_rnr_id,v_program_id, v_product) UNION ALL
select 'total_cost' as rule, * from fn_rule_total_cost(v_rnr_id,v_facility_id, v_product) UNION ALL
select 'consumpton' as rule, * from fn_rule_questionable_consumption(v_rnr_id,v_facility_id, v_product) UNION ALL
select 'losses_and_adjustment' as rule, * from fn_rule_questionable_losses_and_adjustment(v_rnr_id, v_product) UNION ALL
select 'skipped_manageable_commodities' as rule, * from fn_rule_skipped_manageable_health_commodities(v_facility_id, v_rnr_id, v_product, skipped)
;
END;
$$
LANGUAGE plpgsql
