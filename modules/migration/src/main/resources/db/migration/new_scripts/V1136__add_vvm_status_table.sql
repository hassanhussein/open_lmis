DROP TABLE IF EXISTS public.wms_vvm_statuses;
CREATE TABLE public.wms_vvm_statuses
(
  id serial,
  vvmId INTEGER,
  name character varying(30) NOT NULL,
  CONSTRAINT wms_vvm_statuses_pkey PRIMARY KEY (id),
  CONSTRAINT wms_vvm_statuses_name_key UNIQUE (name)
);


DELETE FROM wms_vvm_statuses;
INSERT INTO wms_vvm_statuses (VVMID, NAME)
VALUES(1, 'VVM1'), (2, 'VVM2'), (3, 'VVM3'),(4, 'VVM4');
...
DROP COLUMN failLocationId;

ALTER TABLE inspection_lots
ADD COLUMN failLocationId INTEGER;
...
FOR select r.id,r.note, r.description,r.blawbnumber, r.id receiveid, rli.productid, rli.quantitycounted,rl.boxNumber,rl.locationId, rli.boxcounted,r.invoiceNumber, t.id lotid, rl.lotnumber, rli.lotflag, rl.expirydate, rl.quantity
from receives r
...
VALUES (rec_receive.receiveid, 'DRAFT',NOW(),NOW(),NULL,rec_receive.invoiceNumber, rec_receive.note,rec_receive.description,rec_receive.blawbnumber);
