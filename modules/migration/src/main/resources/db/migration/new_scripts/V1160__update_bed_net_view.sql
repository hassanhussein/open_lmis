...
CREATE MATERIALIZED VIEW public.vw_bed_nets_data AS
 ...
 WITH q AS (
         SELECT facilities.code AS orgunit,
          ...
          WHERE (r.status::text = ANY (ARRAY['APPROVED'::character varying::text, 'RELEASED'::character varying::text])) AND r.emergency = false AND (li.productcode::text IN ( SELECT products_1.code
