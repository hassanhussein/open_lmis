    ...
    public List<? extends Column> getPrintableColumns(Boolean fullSupply, Boolean reportOnlyFlag) {
        ...
        for (Column rnrColumn : columns) {
            if (rnrColumn.getVisible()) {
                if (!reportOnlyFlag && fullSupply && !nonPrintableFullSupplyColumnNames.contains(rnrColumn.getName())) {
                    printableRnrColumns.add((RnrColumn) rnrColumn);
                } else if (!reportOnlyFlag && !fullSupply && printableNonFullSupplyColumnNames.contains(rnrColumn.getName())) {
                    printableRnrColumns.add((RnrColumn) rnrColumn);
                } else if (reportOnlyFlag && fullSupply && printableFullSupplyReportOnlyColumnNames.contains(rnrColumn.getName()))
                {
                    printableRnrColumns.add((RnrColumn) rnrColumn);
                }
            }
