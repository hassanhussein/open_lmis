...
public interface ServiceContractMapper {
  ...
  @Select("select id, name, true as isActive from equipments where id in (select equipmentId from equipment_service_contract_equipment_types where contractId = #{id}) " +
      " UNION " +
      " select id, name, false as isActive from equipments where id not in (select equipmentId from equipment_service_contract_equipment_types where contractId = #{id}) " +
      " order by name")
  ...
  @Select("SELECT * FROM equipment_service_contracts WHERE id IN (SELECT contractId FROM equipment_service_contract_equipment_types WHERE equipmentId = #{equipmentId})")
  ...
  @Insert("INSERT INTO equipment_service_contract_equipment_types ( contractId, equipmentId ) values (#{contractId}, #{equipmentId}) ")
