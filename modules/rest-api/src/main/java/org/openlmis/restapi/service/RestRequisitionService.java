...
public class RestRequisitionService {
  ...
  public Rnr submitFacilityReport(Report report, Long userId) {
    ...
    FacilityInterfaceDTO facilityCode = facilityService.getByAgentCode(report.getAgentCode());

    Facility reportingFacility = facilityService.getOperativeFacilityByCode(facilityCode.getDataSetName());
    Program reportingProgram = programService.getValidatedProgramByCode(report.getProgramCode().toLowerCase());

    List<ProcessingPeriod>periods = requisitionService.getAllPeriodsForInitiatingRequisition(reportingFacility.getId(),reportingProgram.getId());

    Long newPeriodId = 0L;
    newPeriodId = periods.get(0).getId();

    ProcessingPeriod period = processingPeriodService.getById(newPeriodId);
