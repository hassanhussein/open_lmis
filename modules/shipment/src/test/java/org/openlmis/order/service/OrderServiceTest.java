...
public class OrderServiceTest {
  ...
  public void shouldSearchOrdersByStatus() throws Exception {
    ...
    when(orderRepository.searchByWarehousesAndStatuses(asList(4l), asList(RELEASED, PACKED, TRANSFER_FAILED, READY_TO_PACK),1L,0L, commaSeparator.commaSeparateIds(facilities),0L)).thenReturn(expectedOrders);

    List<Order> returnedOrders = orderService.searchByStatusAndRight(3l, asList(RELEASED, PACKED, TRANSFER_FAILED, READY_TO_PACK),1L,0L,0L);
  ...
  public void shouldSortOrdersBasedOnSupplyingFacilityNameProgramNameAndThenCreationDate() throws Exception {
    ...
    when(orderRepository.searchByWarehousesAndStatuses(asList(4l), asList(RELEASED, PACKED, TRANSFER_FAILED, READY_TO_PACK),1L,0L, "{}",0L)).thenReturn(expectedOrders);
