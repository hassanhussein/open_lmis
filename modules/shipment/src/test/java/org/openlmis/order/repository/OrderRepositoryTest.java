...
public class OrderRepositoryTest {
  ...
  public void shouldSearchOrdersByWarehouseIdsAndStatuses() throws Exception {
    ...
    when(orderMapper.getByWarehouseIdsAndStatuses("{3, 6}", "{RELEASED, READY_TO_PACK}",1L, "{}",0L)).thenReturn(expectedOrders);

    List<Order> actualOrders = orderRepository.searchByWarehousesAndStatuses(asList(3l, 6l), asList(RELEASED, READY_TO_PACK),1L,0L, "{}",0L);

    verify(orderMapper).getByWarehouseIdsAndStatuses("{3, 6}", "{RELEASED, READY_TO_PACK}",1L, "{}",0L);
