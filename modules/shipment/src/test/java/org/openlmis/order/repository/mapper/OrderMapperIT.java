...
public class OrderMapperIT {
  ...
  public void shouldGetOrdersBySupplyingFacilityIdsAndStatuses() throws Exception {
    ...
    List<Order> orders = mapper.getByWarehouseIdsAndStatuses("{" + facilityId + "}", "{PACKED}",1L, "{}",0L);

    assertThat(orders.size(), is(0));

    List<Order> noOrders = mapper.getByWarehouseIdsAndStatuses("{" + facilityId + "}", "{RECEIVED}", 1L, "{}",0L);
