...
public class LookupController extends BaseController {
  ...
  public ResponseEntity storeDailyStockStatusFromMSD(@RequestBody String jsonString,
                                                     HttpServletRequest request){
    ObjectMapper mapper = new ObjectMapper();
    String response = "";
    try {
      List<MSDStockDTO> msdStockDTOS = mapper.readValue(jsonString, new TypeReference<List<MSDStockDTO>>() {
      });
      response = lookupService.insertDailyMSDStockStatus(msdStockDTOS, loggedInUserId(request));
    } catch (IOException e) {
      e.printStackTrace();
    }
    return RestResponse.response("Status", response);
  }
