...
public class RequisitionControllerTest {
  ...
  public void shouldInitiateRnr() throws Exception {
    ...
    when(requisitionService.initiate(facility, program, USER_ID, false, null,"WEB_UI")).thenReturn(initiatedRnr);
    ...
    verify(requisitionService).initiate(facility, program, USER_ID, false, null,"WEB_UI");
  ...
  public void shouldGiveErrorIfInitiatingFails() throws Exception {
    ...
    doThrow(new DataException(errorMessage)).when(requisitionService).initiate(new Facility(1L), new Program(2L), USER_ID, false, null,"WEB_UI");
    ResponseEntity<OpenLmisResponse> response = controller.initiateRnr(1L, 2L, false, request);
  ...
  public void shouldReturnModelAndViewForPrintingRequisitionAsPdf() {
    ...
    ModelAndView modelAndView = controller.printRequisition(rnrId,true);
  ...
  public void shouldSetStatusChangesInModelForPrint() throws Exception {
    ...
    ModelAndView printModel = controller.printRequisition(1L,true);
