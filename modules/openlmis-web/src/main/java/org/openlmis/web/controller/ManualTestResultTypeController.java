...
@Controller
@RequestMapping(value="/manualTestResultTypes/")
public class ManualTestResultTypeController extends BaseController {

    public static final String MANUAL_TEST_TYPE = "manualTestResultType";

    @Autowired
    ManualTestResultTypeService manualTestService;
    ...
    @PreAuthorize("@permissionEvaluator.hasPermission(principal,'MANAGE_MANUAL_TEST_TYPES')")
    public ManualTestResultType getManualTestTypeById(@PathVariable(value="id") Long id){
        return manualTestService.getById(id);
    }

    @RequestMapping(value="types",method=POST, headers = ACCEPT_JSON)
    @PreAuthorize("@permissionEvaluator.hasPermission(principal,'MANAGE_MANUAL_TEST_TYPES')")
    public ResponseEntity<OpenLmisResponse> save(@RequestBody ManualTestResultType testType, HttpServletRequest request){
        ...
        successResponse = success(String.format("Test Result type '%s' has been successfully saved", testType.getName()));
    ...
    public ResponseEntity<OpenLmisResponse> remove(@PathVariable(value="tid") Long id, HttpServletRequest request){
        ...
        try {
            ...
            return error("Can not delete test result type. Its already in use", HttpStatus.BAD_REQUEST);
