 ...
 */
function ListProductsController($scope, ProductReportList, $routeParams) {
    $scope.perioderror = "";
    $scope.allReportType = false;
    $scope.OnFilterChanged = function () {
        var filterParam=$scope.getSanitizedParameter();
        ProductReportList.get(filterParam, function (data) {
    ...
    var generateParentChildReport = function(data) {
        ...
        _.each(val, function(row){
            row.programProgramReportList = _.chain(data).where({id: row.id}).map(function(row){ return {'programName': row.programName};}).value();
        });
        return val;
    };
    ...
    function getReportType() {
        ...
        if ($scope.filter === undefined) {
            $scope.filter = {productStatus: reportType};
        } else {
            $scope.filter.productStatus = reportType;
        }
    ...
    $scope.exportReport = function (type) {
        ...
        var url = '/reports/download/product-list-report/' + type + '?' + params;
