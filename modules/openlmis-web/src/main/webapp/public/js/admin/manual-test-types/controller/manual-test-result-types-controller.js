...
function ManualTestResultTypeController($scope, $routeParams, ManualTestResultType, ManualTestResultCategory, $location, $dialog, $route, messageService) {


    $scope.viewMode = $route.current.$$route.mode;
    ManualTestResultCategory.get(function (data) {
        $scope.manualTestCategoryList = data.manualTestResultCategoryList;
    });

    if ($scope.viewMode === 'LIST') {
        ManualTestResultType.get(function (data) {
            $scope.manualTestTypeList = data.manualTestResultTypeList;
        });
    }
        ...
        ManualTestResultType.get({'tid': manualTestTypeId},
            function (response) {
                $scope.testType = response.manualTestResultType;
            });
    }
    else //NEW
    {
    ...
    $scope.saveManualTestType = function () {
        ManualTestResultType.save($scope.testType, function (response) {
                $scope.$parent.message = response.success;
                $location.path('list-result-type');
            },
    ...
    $scope.removeManualTestTypeConfirm = function (result) {
        if (result) {
            ...
            ManualTestResultType.delete({'tid': $scope.testType.id}, function (data) {
