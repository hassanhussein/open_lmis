...
public class FacilityService {
    ...
    public void update(Facility facility) {
        ...
        facilityOwnerService.addFacilityOnwers(facility, facility.getOwners());
    }
    ...
    public FacilityType getFacilityTypeByFac(Long facilityId) {
        return facilityRepository.getFacilityTypeByFac(facilityId);
    }

    public Facility getByDistrict(Long districtId) {

        GeographicZone zone = geographicZoneService.getById(districtId);
        if (zone.getLevel().getId() == 3L) {
            return facilityRepository.getFacilityByParentGeoZone(districtId);
        } else
            return getByGeographicZoneId(districtId, zone.getLevel().getId());
