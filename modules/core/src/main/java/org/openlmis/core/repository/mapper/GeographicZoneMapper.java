...
public interface GeographicZoneMapper {
    ...
    @Select(" SELECT * FROM geographic_zone_map_mappings WHERE GEOGRAPHICZONEID = #{geographicZoneId} ")
    GeoZoneMapDTO getGeoMappingBy(@Param("geographicZoneId") Long geographicZoneId);

    @Insert("INSERT INTO public.geographic_zone_map_mappings(\n" +
            "            geographiczoneid, mapcode)\n" +
            "    VALUES ( #{geographicZoneId}, #{mapCode});")
    @Options(useGeneratedKeys = true)
    Integer saveGeoZoneMap(GeoZoneMapDTO record);

    @Update(" update geographic_zone_map_mappings set geographiczoneid= #{geographicZoneId}, mapCode=#{mapCode}  where id =#{id}")
    void updateGeoZoneMap(GeoZoneMapDTO record);

    @Select("select gz.id,gz.code, m.mapCode from geographic_zone_map_mappings m  " +
