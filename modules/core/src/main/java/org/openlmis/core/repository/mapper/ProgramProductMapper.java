...
public interface ProgramProductMapper {
    ...
    List<ProgramProduct> getByProgramAndCategory(@Param("programId")Long programId, @Param("categoryId") Long categoryId);

    @Select({"with q as( " +
            " select productid from program_products pp " +
            " where pp.programid = #{programId} and pp.productcategoryid = #{categoryId}  " +
            " ) " +
            " select * from products p " +
            " where p.id not in (  " +
            " select productid from q " +
            ")"})

    List<Product> getUnCategorizedProducts( @Param("categoryId") Long categoryId,@Param("programId")Long programId);
}